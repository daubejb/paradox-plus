...
- **Board Click Hit Detection Responsiveness & Mobile Touch (2026-06-13):** Increased the click distance threshold in `interaction.rs` from `60.0` to `85.0` logical pixels. This expands the hit detection zone to cover the full width of curved track cells (96.0 thickness) including the outer boundaries of curved corner cells (specifically fixing wager placement on `7 FW` on Hole 5). Integrated Bevy's `Touches` resource to support mobile touch tap coordinates alongside desktop mouse clicks, resolving mobile input unresponsiveness. Added touch-specific integration test coverage. All unit, integration, and UI tests pass.
- **Wager Card Button Selection & Visual Refinements (2026-06-13):** Visual selection highlights (thick gold borders) were implemented for active wager cards, and zero-quantity card selection buttons are now visually desaturated, faded, and functionally disabled. Clicking a selected card toggles its selection state. Prevented WASM memory churn and UI layout thrashing by checking for scorecard/selection change detection flags before rendering. Re-verified loopback server transition logic for practice matches starting with zero inventory. All unit, integration, and UI tests compile and pass successfully.
- **FSM StateSync Player Movement Direction Replication (2026-06-13):** Resolved a bug where players were unable to click cells to slide 0-4 spaces during the Trickster Banana choice phase when overshooting the green (clamping at space 7). Added a replicated `player_directions` field to `ServerUpdate::StateSync` and populated it in all loopback and broadcast systems. Client-side replication now correctly initializes the replicated `Ball` entity's `direction` field, enabling direction-aware path rendering and slide selection clicks. All tests compile and pass.
- **iOS TestFlight Deployment & Signing Fix (2026-06-13):** Resolved TestFlight build export and upload issues. Aligned bundle identifier to `com.kahndaube.paradoxgolf` dynamically via `Info.plist` and `generate_xcodeproj.py`, configured iPad multitasking requires-fullscreen settings, stripped alpha channel transparency from the 1024x1024 app icon, and successfully uploaded build version 21 IPA package to TestFlight.
- **Code Hygiene & Bevy 0.14 Deprecation Cleanup (2026-06-14):** Refactored client setup screen input to use modern `KeyboardInput` and `Key` APIs, eliminating Bevy's deprecated `ReceivedCharacter` compiler warnings. Removed the unused `SafeFixedMath` trait from the protocol crate and converted its corresponding unit tests to use native saturating and checked mathematical operations. Verified WASM target compatibility. All unit, integration, and UI tests pass.
