...
- **Board Click Hit Detection Responsiveness (2026-06-13):** Increased the click distance threshold in `interaction.rs` from `60.0` to `85.0` logical pixels. This expands the hit detection zone to cover the full width of curved track cells (96.0 thickness) including the outer boundaries of curved corner cells (specifically fixing wager placement on `7 FW` on Hole 5). Added integration test coverage. All unit, integration, and UI tests pass.
